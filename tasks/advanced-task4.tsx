...
export default function VisualSequenceEncodingChallenge({ onComplete, onUnlockNext }: VisualSequenceEncodingChallengeProps) {
    ...
    <div className="grid grid-cols-5 gap-1">
      {Array.from({ length: GRID_SIZE * GRID_SIZE }).map((_, index) => {
        ...
        return (
          <motion.div
            key={index}
            className={`w-10 h-10 border ${selectedCell && selectedCell[0] === row && selectedCell[1] === col ? 'border-indigo-500' : 'border-gray-300'} rounded-md flex items-center justify-center cursor-pointer ${element ? 'bg-gray-100' : 'hover:bg-gray-50'} relative`}
            ...
            {element && (
              <>
                {React.createElement(shapes[element.shape], {
                  className: `w-6 h-6 ${colorClasses[element.color]}`,
                })}
                {!showSequence && (
                  <motion.button
  ...
  return (
    <div className="flex flex-col items-center justify-center p-4 space-y-4 bg-white rounded-xl shadow-lg max-w-md mx-auto">
      ...
      {!showSequence && (
        <div className="flex flex-col items-center space-y-2">
          ...
          <Button 
            ...
            className="px-4 py-1 bg-indigo-600 text-white text-sm rounded-full hover:bg-indigo-700 transition-colors duration-200"
          >
            Submit
          </Button>
        </div>
